...
of time steps required for a player to escape from a flooding grid. As an input, we are given a $n\times n$ grid with the symbols already filled out, (see 'The problem' for description of this grid). \\\\
...
source) (basically recreating our input), together with two initially unset time fields: \texttt{flood\_turn}
...
\begin{algorithm}[H]
\caption{Initialization of \texttt{tileGrid} and \texttt{waterQueue}}
\begin{algorithmic}[1]

\State Create an empty two--dimensional array $\texttt{tileGrid}[0 \ldots n-1][0 \ldots n-1]$
\State Create an empty queue $\texttt{waterQueue}$

\For{$i \gets 0$ \textbf{to} $n-1$}
    \For{$j \gets 0$ \textbf{to} $n-1$}

        \State Read the character \texttt{c} from the input grid at position $(i,j)$
        \State Create a new node \texttt{n} with fields:
            \Statex \hspace{2em} \texttt{n.value} $\gets$ \texttt{c}
            \Statex \hspace{2em} \texttt{n.flood\_turn} $\gets - 1$ 
            \Statex \hspace{2em} \texttt{n.reach\_turn} $\gets -1$
            \Statex \hspace{2em} \texttt{n.x} $\gets i$, \; \texttt{n.y} $\gets j$
        \State $\texttt{tileGrid}[i][j] \gets \texttt{n}$

        \If{\texttt{c} is a water source}
            \State \texttt{n.flood\_turn} $\gets 0$
            \State Enqueue $(i,j)$ into $\texttt{waterQueue}$
        \EndIf
    \EndFor
\EndFor
...
Psuedocode:
\begin{algorithm}[H]
\caption{Flood Propagation Using Breadth--First Search}
\begin{algorithmic}[1]

\While{$\texttt{waterQueue}$ is not empty}

    \State Dequeue a cell $(x,y)$ from $\texttt{waterQueue}$
    \State Let $u \gets \texttt{tileGrid}[x][y]$

    \For{each direction $(\Delta x, \Delta y)$ in $\{(-1,0), (1,0), (0,1), (0,-1)\}$}

        \State $x' \gets x + \Delta x$, \quad $y' \gets y + \Delta y$

        \If{$(x',y')$ is within bounds}
            \State Let $v \gets \texttt{tileGrid}[x'][y']$

            \If{$v.\texttt{value}$ is not a rock \textbf{and} $v.\texttt{flood\_turn} = -1$}
                \State $v.\texttt{flood\_turn} \gets u.\texttt{flood\_turn} + 1$
                \State Enqueue $(x',y')$ into $\texttt{waterQueue}$
            \EndIf
...
\begin{algorithm}[H]
\caption{Player Movement Using Breadth--First Search}
\begin{algorithmic}[1]

\State Insert the player’s starting cell $(s_x, s_y)$ into $\texttt{distanceQueue}$
\State $\texttt{tileGrid}[s_x][s_y].\texttt{reach\_turn} \gets 0$

\While{$\texttt{distanceQueue}$ is not empty}

    \State Dequeue a cell $(x,y)$ from $\texttt{distanceQueue}$
    \State Let $u \gets \texttt{tileGrid}[x][y]$

    \If{$u$ is the shelter cell}
        \State \Return $u.\texttt{reach\_turn}$ \Comment{player escapes}
    \EndIf
    ...
    \For{each direction $(\Delta x, \Delta y)$ in $\{(-1,0), (1,0), (0,1), (0,-1)\}$}
        ...
        \If{$(x',y')$ is within bounds}
            ...
            \State Compute arrival time $t \gets u.\texttt{reach\_turn} + 1$
            \State Let $\textit{flooded} \gets (v.\texttt{flood\_turn} \neq -1$ \textbf{and} $v.\texttt{flood\_turn} \le t)$

            \If{$v.\texttt{value}$ is a walkable cell \textbf{and} $v.\texttt{reach\_turn} = -1$ \textbf{and not} $\textit{flooded}$}
                \State $v.\texttt{reach\_turn} \gets t$
                \State Enqueue $(x',y')$ into $\texttt{distanceQueue}$
            \EndIf
